        ...
        {filteredUsers.length === 0 ? (
          ...
          filteredUsers.map((user) => (
            <div key={user.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
              ...
              {/* Details Grid */}
              <div className="grid grid-cols-2 gap-3 mb-3 py-3 border-t border-gray-100">
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">{user.total_appointments || 0}</p>
                </div>
              ...
              <div className="flex gap-2 pt-3 border-t border-gray-100">
                {user.status === 'suspended' ? (
                  <button
                    onClick={() => activateUser(user.id, getDisplayName(user))}
                    className="flex-1 px-3 py-2 bg-green-100 text-green-700 hover:bg-green-200 rounded-lg font-medium transition text-sm"
                  >
                    Activate
                  </button>
                ) : (
